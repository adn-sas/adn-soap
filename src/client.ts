...
export class Client extends EventEmitter {
  ...
  private _defineService(service: ServiceElement, endpoint?: string) {
    ...
    for (const name in ports) {
      const httpsLocation = ports[name].location[4] !== 's' && ports[name].location.indexOf('demo') === -1 ? ports[name].location.replace('http','https') : ports[name].location;
  ...
  private _invoke(method: OperationElement, args, location: string, callback, options, extraHeaders) {
    ...
    xml = '<?xml version="1.0" encoding="utf-8"?>' +
      ...
      'xmlns:ser1="http://schemas.datacontract.org/2004/07/ServiceSoap.UBL2._0.Models.Object" ' +
      'xmlns:arr="http://schemas.microsoft.com/2003/10/Serialization/Arrays" ' +
      'xmlns:tem="http://tempuri.org/" ' +
      'xmlns:wcf="http://schemas.datacontract.org/2004/07/WcfRecepcionSOAP" ' +
      encoding +
